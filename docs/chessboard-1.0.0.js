...
;(function () {
  ...
  function validPieceCode (code) {
    return isString(code) && code.search(/^[bw][KQRNBPA]$/) !== -1
  }
  ...
  if (RUN_ASSERTS) {
    ...
    console.assert(validPieceCode('wA'))
    console.assert(validPieceCode('bA'))
    console.assert(!validPieceCode('WR'))
    console.assert(!validPieceCode('Wr'))
  ...
  function validFen (fen) {
    ...
    for (var i = 0; i < 8; i++) {
      if (chunks[i].length !== 8 ||
          chunks[i].search(/[^kqrnbpaKQRNBPA1]/) !== -1) {
        return false
      }
    }
  ...
  function constructor (containerElOrString, config) {
    ...
    function buildSparePiecesHTML (color) {
      var pieces = ['wK', 'wQ', 'wR', 'wB', 'wN', 'wP', 'wA']
      if (color === 'black') {
        pieces = ['bK', 'bQ', 'bR', 'bB', 'bN', 'bP', 'bA']
      }
